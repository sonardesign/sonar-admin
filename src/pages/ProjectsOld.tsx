...
export const Projects: React.FC = () => {
  ...
  useEffect(() => {
    if (clients.length > 0 && expandedClients.size === 0) {
      ...
      // Also include "No Client" if there are projects without clients
      const projectsWithoutClient = projects.filter(p => !p.client_id && !p.clientId);
      if (projectsWithoutClient.length > 0) {
        allClientNames.add('No Client');
      }
  ...
  const handleCreateProject = async () => {
    if (newProjectName.trim() && selectedClientId) {
      ...
      if (client) {
        console.log('📝 Creating project:', { name: newProjectName.trim(), color: selectedColor, clientId: selectedClientId })
        
        try {
          const result = await createProject(newProjectName.trim(), selectedColor, selectedClientId);
          if (result) {
            console.log('✅ Project created successfully:', result.id)
            
            // Expand the client group so the new project is visible
            const clientName = result.client_name || result.clientName || 'No Client';
            ...
            toast.error('Failed to create project. Please check console for details.')
  ...
  console.log('🔍 Projects page debug:', {
    ...
    projects: projects.map(p => ({
      ...
      name: p.name,
      client_id: p.client_id?.substring(0, 8) || 'NULL',
      client_name: p.client_name || p.clientName || 'MISSING',
      is_archived: p.is_archived || p.archived
    })),
  ...
  const groupedProjects = filteredProjects.reduce((groups, project) => {
    const clientName = project.client_name || project.clientName || 'No Client';
  ...
  // Ensure "No Client" exists in grouped projects if there are projects without clients
  const projectsWithoutClient = filteredProjects.filter(p => !p.client_id && !p.clientId);
  if (projectsWithoutClient.length > 0 && !allGroupedProjects['No Client']) {
    allGroupedProjects['No Client'] = projectsWithoutClient;
  }

  // Ensure "No Client" is expanded if it has projects
  useEffect(() => {
    const projectsWithoutClient = projects.filter(p => !p.client_id && !p.clientId && !(p.is_archived || p.archived));
    if (projectsWithoutClient.length > 0) {
      setExpandedClients(prev => {
        if (!prev.has('No Client')) {
          const newSet = new Set(prev);
          newSet.add('No Client');
          return newSet;
        }
        return prev;
      });
  ...
  console.log('📊 Grouped projects:', {
    keys: Object.keys(allGroupedProjects),
    counts: Object.entries(allGroupedProjects).map(([key, projs]) => ({ client: key, count: projs.length })),
    totalProjects: filteredProjects.length,
    projectsWithoutClient: projectsWithoutClient.length
  });
